  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6 sm:px-5 sm:py-8">
      ...
      <p className="mt-5 max-w-2xl border-t pt-5 text-[15px] leading-6 text-ink-1 rule">
        ...
        hole is traced on its own frame from the shot coordinates the capture
        carries — drawn here, not copied from anywhere.
      </p>
      ...
      {heard.map((r) => (
        <RoundEntry key={r.scorecardId} round={r} grint={linked.get(r.scorecardId) ?? null} />
      ))}
...
function RoundEntry({ round, grint }: { round: GarminRound; grint: PlayedRound | null }) {
...
function HoleEntry({ hole, grintPutts }: { hole: GarminHole; grintPutts: number | null }) {
...
/* Drawn from the shot coordinates on Garmin's per-hole map frame (tee low,
 * green high — y already grows downward, like SVG). The frame's raster image
 * is Garmin's and is never fetched; these are the record's own numbers on a
 * turf-coloured card of ours. Solid segments are shots; a dotted segment is
 * the gap between where one shot ended and the next began — a walk, a drop,
 * or the cartography disagreeing with itself. */
function HoleTrace({ shots }: { shots: GarminShot[] }) {
  const segs = shots
        ...
        return (
          <g key={i}>
            ...
            <line
              ...
              x2={s.b.x}
              y2={s.b.y}
              stroke="var(--turf-tick)"
              strokeWidth={1.5}
              vectorEffect="non-scaling-stroke"
            />
            <circle cx={s.b.x} cy={s.b.y} r={r} fill="var(--turf-tick)" />
          </g>
        );
      })}
