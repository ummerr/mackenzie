 ...
 * from the retired /diary page: the components are the same, only the shell
 * around them changed. */
...
export function WatchSection({
  ...
  if (garmin === null) {
    return (
      <section className="mt-10" id="watch">
        <h2 className="font-serif text-[26px] leading-tight">What the watch heard</h2>
        <p className="mt-2 max-w-2xl font-mono text-[11px] leading-5 text-ink-3">
          ...
          extension, then{" "}
          <code className="text-ink-2">pnpm data:garmin</code>.
        </p>
      </section>
    );
  }
  ...
  return (
    <section className="mt-10" id="watch">
      <h2 className="font-serif text-[26px] leading-tight">What the watch heard</h2>
      <p className="mt-2 max-w-2xl font-mono text-[11px] leading-5 text-ink-3">
        {heard.length} round{heard.length === 1 ? "" : "s"} · {totalShots} shots.
        Every AutoShot the watch recorded, hole by hole, traced over the hole
        itself — the shots from the coordinates the capture carries, laid on the
        same satellite imagery the course map stands on. The marks are drawn
        here; nothing is copied from Garmin. Imagery &copy; Esri, Maxar,
        Earthstar Geographics — World Imagery tiles, loaded by your browser from
        Esri&rsquo;s public service.
      </p>
